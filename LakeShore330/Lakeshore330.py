...
class LakeShore330Gpib:
    """Minimal Lake Shore 330 IEEE-488 interface used by this GUI."""
    ...
    def __init__(self, resource_name, timeout_ms=2000):
    ...
    def configure_communications(self):
        # The Model 330 supports CR/LF output termination as TERM 0.
        self.command("TERM 0")
    ...
    def prepare_kelvin_monitoring(self, control_channel=None):
        ...
        self.command("CUNI K")
        time.sleep(0.55)
        self.command(f"SCHN {sample_channel}")
        time.sleep(0.55)
        self.command("SUNI K")
        time.sleep(0.55)
    ...
    def get_input_type_label(self, channel):
        input_type = self.query(f"{channel.upper()}TYPE?").strip().upper()
        return self.INPUT_TYPE_LABELS.get(input_type, input_type)
    ...
    def set_heater_pid(self, _output, gain, integral, derivative):
        self.command(f"GAIN {gain}")
        self.command(f"RSET {integral}")
        self.command(f"RATE {derivative}")

    def get_heater_pid(self):
        return [self.query("GAIN?"), self.query("RSET?"), self.query("RATE?")]

    def get_heater_range(self, _output=1):
        return self.HeaterRange(int(self.query("RANG?")))

    def set_heater_range(self, _output, heater_range):
        self.command(f"RANG {int(heater_range)}")

    def get_heater_output_percent(self):
        return float(self.query("HEAT?"))

    def get_tune_mode_text(self):
        return self.TUNE_MODE_LABELS.get(self.query("TUNE?").strip(), "Unknown")

    def set_manual_pid_mode(self):
        self.command("TUNE 0")
...
class LakeShoreGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Lake Shore 330: GPIB Controller")
    ...
    def _init_ui(self):
        ...
        self.sensor_type_names = list(LakeShore330Gpib.INPUT_TYPE_LABELS.values())
        self.sensor_unit_names = ["KELVIN"]
        self.range_options = {
            "Silicon Diode": [],
            "Platinum RTD": [],
            "Cernox / NTC RTD": [],
        ...
        ttk.Label(conn_frame, text="GPIB Resource:").pack(anchor="w", padx=5, pady=(5, 0))
        self.ent_gpib_resource = ttk.Entry(conn_frame, textvariable=self.gpib_resource_var)
        self.ent_gpib_resource.pack(fill="x", padx=5, pady=2)
        ...
        det_frame = ttk.LabelFrame(admin_tab, text="Detector Status (Model 330)")
        ...
        self.combo_detector_type = ttk.Combobox(
            det_frame, textvariable=self.detector_type_var, values=self.sensor_type_names, state="disabled", width=18
        )
        ...
        self.combo_detector_units = ttk.Combobox(
            det_frame, textvariable=self.detector_units_var, values=self.sensor_unit_names, state="disabled", width=18
        )
        ...
        self.detector_autorange_check = ttk.Checkbutton(
            det_frame, text="Autorange", variable=self.detector_autorange_var, command=self.on_detector_autorange_toggle
        )
        self.detector_autorange_check.grid(row=4, column=0, sticky="w", padx=4, pady=2)
        self.detector_autorange_check.config(state="disabled")
        self.detector_comp_check = ttk.Checkbutton(
            det_frame, text="Compensation", variable=self.detector_comp_var
        )
        self.detector_comp_check.grid(row=4, column=1, sticky="w", padx=4, pady=2)
        self.detector_comp_check.config(state="disabled")

        ttk.Button(det_frame, text="Load Channel", command=self.load_detector_config).grid(row=5, column=0, sticky="we", padx=4, pady=4)
        self.btn_apply_detector_config = ttk.Button(det_frame, text="Apply Config", command=self.apply_detector_config, state="disabled")
        self.btn_apply_detector_config.grid(row=5, column=1, sticky="we", padx=4, pady=4)
        ttk.Label(
            det_frame,
            text="Input type/range is configured on the Model 330 hardware; this panel is read-only.",
            foreground="dim gray",
            wraplength=265,
            justify="left",
        ...
        self.btn_heater_load = ttk.Button(cfg_frame, text="Heater Load: Set On Hardware", style="Warning.TButton", state="disabled")
        self.btn_heater_load.pack(fill="x", padx=5, pady=2)
    ...
    def _format_status(self, status_obj):
        if getattr(status_obj, "status_available", None) is False:
            return "Not reported by Model 330"
        active_flags = [
            field.replace("_", " ")
    ...
    def _heater_range_power_factor(self, heater_range):
        power_factors = {
            LakeShore330Gpib.HeaterRange.OFF: 0.0,
            LakeShore330Gpib.HeaterRange.LOW: 0.01,
            LakeShore330Gpib.HeaterRange.MEDIUM: 0.1,
            LakeShore330Gpib.HeaterRange.HIGH: 1.0,
        }
    ...
    def _get_active_heater_range(self):
        try:
            ...
            range_map = {
                "OFF": LakeShore330Gpib.HeaterRange.OFF,
                "LOW": LakeShore330Gpib.HeaterRange.LOW,
                "MEDIUM": LakeShore330Gpib.HeaterRange.MEDIUM,
                "HIGH": LakeShore330Gpib.HeaterRange.HIGH,
            }
            return range_map.get(self.combo_range.get(), LakeShore330Gpib.HeaterRange.HIGH)
    ...
    def _update_heater_limit_label(self):
    ...
    def _set_heater_range_enum(self, heater_range):
        ...
        range_names = {
            ...
            LakeShore330Gpib.HeaterRange.LOW: "LOW",
            LakeShore330Gpib.HeaterRange.MEDIUM: "MEDIUM",
            LakeShore330Gpib.HeaterRange.HIGH: "HIGH",
        }
    ...
    def _auto_adjust_program_range(self, current_temp, target_temp):
        ...
        if estimated_power_w <= self.max_heater_power_w * 0.01:
            heater_range = LakeShore330Gpib.HeaterRange.LOW
        elif estimated_power_w <= self.max_heater_power_w * 0.1:
            heater_range = LakeShore330Gpib.HeaterRange.MEDIUM
        else:
            heater_range = LakeShore330Gpib.HeaterRange.HIGH

        if current_temp is not None:
            error = abs(target_temp - current_temp)
            if error > 10:
                heater_range = LakeShore330Gpib.HeaterRange.HIGH
            elif error > 3 and heater_range == LakeShore330Gpib.HeaterRange.LOW:
                heater_range = LakeShore330Gpib.HeaterRange.MEDIUM
    ...
    def _update_sensor_metadata(self):
        for channel in self.sensor_channels:
            try:
                sensor_desc = f"{self.ls.get_input_type_label(channel)} / K"
            except Exception:
                sensor_desc = "Unavailable"

            self.sensor_rows[channel]["name"].config(text=f"Channel {channel}")
    ...
    def load_detector_config(self, _event=None):
        ...
        try:
            self.detector_type_var.set(self.ls.get_input_type_label(channel))
            self.detector_units_var.set("KELVIN")
            self.detector_autorange_var.set(False)
            self.detector_comp_var.set(False)
            self.detector_range_var.set("")
            self._refresh_detector_range_options()
    ...
    def apply_detector_config(self):
        messagebox.showinfo(
            "Detector Config",
            "Model 330 input type and range are configured on the instrument hardware.",
        )
    ...
    def _update_sensor_readings(self):
        ...
        for channel in self.sensor_channels:
            ...
            self.sensor_rows[channel]["raw"].config(
                text="-" if raw_value is None or not math.isfinite(raw_value) else f"{raw_value:.3f}"
            )
    ...
    def capture_channel_snapshot(self):
        ...
        for channel in self.sensor_channels:
            ...
            temp_value = temperatures.get(channel)
            raw_value = raw_readings.get(channel)
            temp_text = "-" if temp_value is None or not math.isfinite(temp_value) else f"{temp_value:.3f} K"
            raw_text = "-" if raw_value is None or not math.isfinite(raw_value) else f"{raw_value:.3f}"
    ...
    def connect_instrument(self):
        ...
        try:
            ...
            self.ent_gpib_resource.config(state="disabled")
            ...
            # Read PID
            try:
                pid_str = self.ls.get_heater_pid()
    ...
    def set_manual_out(self):
        messagebox.showinfo(
            "Manual Output",
            "Direct manual heater output is not available through the Model 330 command set used here.",
        )
    ...
    def set_heater_load(self, ohm_setting):
        messagebox.showinfo(
            "Heater Load",
            "The Lake Shore 330 heater-load configuration is set on the instrument, not through this GPIB UI.",
        )
