        ...
        for k = 1:nPlots
            ...
            for b = 1:nBins
                ...
                if ~isempty(v)
                    ...
                    % Correct Convergence Math: v is cumsum, so v ./ count = average
                    % We take ABS because Log Scale requires positive numbers
                    cumavg = abs(v(1:len)./(v(1:len)));
            ...
            % Plot with Log Scale
            nexttile; 
            contourf(X, Y, ampMat, 50, 'LineColor','none'); % 50 levels for smooth log gradient
            colormap jet; 
            cb = colorbar; 
            set(gca, 'ColorScale', 'log'); % <--- LOG SCALE ACTIVATED
            title(pairLabels{p});
        end
