function cm_pipeline_contour_save(target_folder)
% ==========================================================
% Quantum Squeezing Pipeline: CONTOUR PLOTS (Saved)
% 
% * Generates Time vs. Position Heatmaps (Contour Plots).
% * FOCUS: Color map is clamped to 0-1000 urad^2.
% * SAVES: Automatically exports PNGs to the data folder.
% * NO VIEWER: Runs entirely automatically.
% ==========================================================
...
frame_dt_s     = 0.025;                
rel_threshold  = 0.20; 
UPPER_LIMIT    = 5000; % The "Stress" Limit
...
fprintf('After filtering: %d pairs valid.\n', size(pairs,1));
...
% Truncate to minimum frame count for uniform grid
min_frames = inf;
for b = 1:nBins
...
fprintf('Grid Size: %d Position Steps x %d Time Steps\n', nBins, min_frames);
...
%% 6. Generate and Save Contour Plots
...
for g = 1:numel(groups)
    ...
    for k = 1:nPlots
        ...
        % === CONTOUR PLOT ===
        % 20 levels helps show gradients clearly
        contourf(X_mesh, Y_mesh, Z, 50, 'LineColor', 'none');
        
        % === STRESSING THE <1000 PART ===
        % 1. Force the Color Axis (CLim) to 0-1000
        clim([0, UPPER_LIMIT]);
        
        % 2. Use a standard heatmap colormap
        colormap(jet);
    ...
    sgtitle(sprintf('Group %d: Amplitude Heatmap (Clipped at %d \\mu rad^2)', g, UPPER_LIMIT), ...
            'FontSize', 14);
    
    % Save
    outName = fullfile(runFolder, sprintf('contour_group%d_limit%d.png', g, UPPER_LIMIT));
    exportgraphics(fBig, outName, 'Resolution', 150);
    fprintf('Saved: %s\n', outName);
    close(fBig);
end

fprintf('All contour plots saved successfully.\n');
