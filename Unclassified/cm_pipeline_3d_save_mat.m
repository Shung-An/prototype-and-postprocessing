function cm_pipeline_3d_save_mat(target_folder)
% ==========================================================
% Quantum Squeezing Pipeline: 3D Surface & MAT Export
% 
% 1. Loads & Filters Data.
% 2. Calculates Converged Running Averages.
% 3. SAVES data to '3d_surface_data.mat' for later use.
% 4. Generates 3D Surface Plots (PNG).
% ==========================================================
...
fprintf('Processing %d bins. Normalizing matrix sizes...\n', nBins);

%% 5. Calculate Running Averages
idx_lin = @(r,c) sub2ind([8 8], r, c);
cumsums = cell(size(pairs,1), nBins); 

% Truncate to minimum frame count to create perfect 3D rectangle
min_frames = inf;
for b = 1:nBins
...
fprintf('Uniform Grid Size: %d Position Steps x %d Time Steps\n', nBins, min_frames);
...
for p = 1:size(pairs,1)
    ...
    for b = 1:nBins
        ...
        % Running Average (Convergence Calculation)
        val = abs(scale_to_urad2(cumsum(diff_sig) ./ (1:min_frames)'));
        cumsums{p,b} = val;
    end
end

%% 6. EXPORT TO MAT FILE (The requested step)
...
% Create Structure
surfaceData = struct();
surfaceData.X_Time_s = X_mesh;
surfaceData.Y_Pos_mm = Y_mesh;
surfaceData.time_vector = time_axis;
surfaceData.position_vector = binVals;
surfaceData.pairs = struct();

% Pack Z-Matrices per Pair
for p = 1:size(pairs,1)
    ...
    surfaceData.pairs(p).label = pairLabels{p};
    surfaceData.pairs(p).Z_Amplitude_urad2 = Z;
end

% Save
savePath = fullfile(runFolder, '3d_surface_data.mat');
save(savePath, 'surfaceData', '-v7.3');
fprintf('SUCCESS: Saved 3D data to: %s\n', savePath);

%% 7. Generate Plots (Visual Check)
...
for g = 1:numel(groups)
    ...
    for k = 1:nPlots
        p = idxList(k);
        % Retrieve Z from our saved struct logic for consistency
        Z = surfaceData.pairs(p).Z_Amplitude_urad2;
        ...
        set(gca, 'ZScale', 'log'); 
        view(-45, 30); axis tight; grid on; colormap jet;
        title(pairLabels{p}, 'Interpreter', 'none');
    end
    exportgraphics(fBig, fullfile(runFolder, sprintf('3d_surface_group%d.png', g)), 'Resolution', 150);
    close(fBig);
end
end
